...
long int tm_get_gmtoff(const struct tm *tm)
{
    time_t tt_now = time(NULL);
    struct tm tm_local = *(localtime(&tt_now));
    struct tm tm_gmt = *(gmtime(&tt_now));
    return mktime(&tm_local) - mktime(&tm_gmt);
}

void tm_set_gmtoff(struct tm *tm, long int gmtoff)
{
    long int prior = tm_get_gmtoff(tm);
    long int diff = gmtoff - prior;
    time_t tt = mktime(tm) - diff;
    *tm = *(localtime(&tt));
}
